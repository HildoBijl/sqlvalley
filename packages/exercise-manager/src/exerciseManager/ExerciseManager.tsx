...
export function ExerciseManager({ showAdminControls = false, ...options }: ExerciseManagerProps) {
	...
	const { registration, instance, loading, generating, submitting, moduleError, generationError, submissionError, retryGeneration, dismissSubmissionError, submitAction, setDraftInput, startNewExercise, selectExerciseById } = useExerciseSession(options)

	// Check situations in which we cannot display the exercise.
	if (moduleError) return <Alert severity="error">{moduleError.message}</Alert>

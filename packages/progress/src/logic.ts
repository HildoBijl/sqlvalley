...
function getSkillSolvedCount(moduleState: ModuleProgressState | undefined): number {
	...
	return solved === undefined ? 0 : ensureInteger(solved, { nonNegative: true })
}
...
export function getRawModuleCompletion(
	...
	for (const module of Object.values(moduleTree)) {
		...
		if (module.type === 'concept') {
			if (isUnderstood(moduleState)) completed.add(module.id)
			continue
		}

		const solved = getSkillSolvedCount(moduleState)
		if (solved > 0) skillProgress[module.id] = solved
		if (isUnderstood(moduleState) || solved >= requiredCount) completed.add(module.id)
	}

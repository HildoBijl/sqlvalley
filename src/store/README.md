...
Exercise instances use the shared format from `@sqlvalley/exercise-instances`. The exercise-instances package generates the complete instance; the learning store stores it, appends submission history, and updates progress. That package also owns instance normalization. Submission pending status is not persisted. Draft input is retained after completion so the submitted answer remains visible. Saving a draft or starting an exercise with a draft checks that it is a plain data object, including its nested values; `undefined` is allowed to clear the draft.

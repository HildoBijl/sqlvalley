...
Exercise instances use the shared format from `@sqlvalley/exercise-instances`. The exercise-instances package generates the complete instance; the learning store stores it, appends submission history, and updates progress. That package also owns instance normalization. Submission pending status is not persisted.

...
function readPersistedRoot(raw: string): { state: RecordValue; version?: number } | null {
	try {
		...
		return { state, version: typeof parsedRecord.version === 'number' ? parsedRecord.version : undefined }
	} catch {
		return null
	}
}
...
function pickSettingsStateFromLegacyRoot(legacyState: RecordValue): RecordValue {
	...
	if (source.currentTheme === 'light' || source.currentTheme === 'dark') result.currentTheme = source.currentTheme
	if (typeof source.hideStories === 'boolean') result.hideStories = source.hideStories
	if (source.practiceDatasetSize === 'full' || source.practiceDatasetSize === 'small') result.practiceDatasetSize = source.practiceDatasetSize
	if (typeof source.hasAccessedPlanningMode === 'boolean') result.hasAccessedPlanningMode = source.hasAccessedPlanningMode

	const goalNodeID = asRecord(source.goalNodeID)
	if (Object.keys(goalNodeID).length > 0) result.goalNodeID = Object.fromEntries(Object.entries(goalNodeID).filter(([treeId, goalNodeId]) => treeId.length > 0 && (typeof goalNodeId === 'string' || goalNodeId === null)))
	return result
}
...
function pickLearningStateFromLegacyRoot(legacyState: RecordValue): RecordValue {
	...
	const modulesCandidate = Object.keys(modulesSource).length > 0 ? modulesSource : componentsSource

	const safeModules = Object.fromEntries(Object.entries(modulesCandidate).filter(([id, component]) => id.length > 0 && Boolean(component) && typeof component === 'object' && !Array.isArray(component)))
	return Object.keys(safeModules).length > 0 ? { modules: safeModules } : {}
}

function writeVersionedState(storage: Storage, key: string, state: RecordValue, version: number): void {
	storage.setItem(key, JSON.stringify({ state, version }))
}

function readDomainStateFromSplitStorage(storage: Storage, legacySplitKey: string): RecordValue | null {
...
function readDomainStateFromLegacyMonolith(storage: Storage, domain: MigrationDomain): RecordValue | null {
	...
	const pickedState = domain === 'settings' ? pickSettingsStateFromLegacyRoot(parsedLegacy.state) : pickLearningStateFromLegacyRoot(parsedLegacy.state)
...
export function migrateLegacyStorageIfNeeded({ domain, targetKey, legacySplitKey, targetVersion }: LegacyMigrationOptions): void {
	if (hasRunLegacyMigrationByDomain[domain] || typeof window === 'undefined') return
	...
	try {
		...
		if (hasLegacyMarker) storage.setItem(SPLIT_STORAGE_MIGRATION_KEY, '1')

		const migratedFromMonolith = readDomainStateFromLegacyMonolith(storage, domain)
		if (migratedFromMonolith) writeVersionedState(storage, targetKey, migratedFromMonolith, targetVersion)

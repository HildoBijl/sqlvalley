...
function normalizeStoredExerciseEvent(
	value: unknown,
): StoredExerciseEvent | null {
	...
	return {
		...
		resultingState: isRecord(value.resultingState)
...
function normalizeStoredExerciseInstance(
	value: unknown,
): StoredExerciseInstance | null {
	if (!isRecord(value)) return null

	const exerciseIdRaw = value.exerciseId
	const exerciseId =
		typeof exerciseIdRaw === 'string' ? exerciseIdRaw.trim() : ''
...
export function normalizeConceptModuleState(
	...
	const common = normalizeCommonModuleFields(
		id,
		state as LegacyModuleShape | undefined,
	)
	return {
		...base,
		...common,
	...
	const normalized: SkillModuleState = {
		...
		exercises: Array.isArray(partialSkill?.exercises)
			? partialSkill.exercises
				...
				.filter(
					(exercise): exercise is StoredExerciseInstance => exercise !== null,
				)
			: [],
	}
	return normalized
...
function looksLikeSkillModule(
	state: Partial<ModuleState> & { type?: unknown },
): boolean {
	return (
		state.type === 'skill' ||
...
export function normalizeModuleState(
	...
	if (!state) return createModuleState(id, 'skill')
	const stateRecord = state as Partial<ModuleState> & { type?: unknown }
	if (looksLikeSkillModule(stateRecord)) return normalizeSkillModuleState(id, stateRecord)

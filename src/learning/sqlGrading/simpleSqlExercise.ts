...
export interface SimpleSqlExerciseState {
  id: string;
  version: number;
  prompt: string;
  solution: string;
...
export function buildSimpleSqlExerciseModule(exercises: SimpleSqlExercise[]) {
  ...
  const toExerciseState = (exercise: SimpleSqlExercise): SimpleSqlExerciseState => {
  ...
  function generate(
    utils: SqlExerciseHelpers,
    context?: { previousExercise?: SimpleSqlExerciseState | null },
  ): SimpleSqlExerciseState {
    ...
    const available =
      previousId && exercises.length > 1
        ...
        : exercises;
    const exercise = utils.selectRandomly(available as readonly SimpleSqlExercise[]);
    return toExerciseState(exercise);
  }

  function getDescription(exercise: SimpleSqlExerciseState): string {
    return exercise.description || exercise.prompt;
  }

  function validateInput(
    args: SqlInputValidationArgs<SimpleSqlExerciseState, string, unknown>,
  ): SqlValidationResult {
  ...
  function validateOutput(
    _exercise: SimpleSqlExerciseState,
    result: SqlExecutionResult<QueryResult[]>,
  ): SqlValidationResult {
    if (!result.success) {
  ...
  function verifyOutput(
    exercise: SimpleSqlExerciseState,
  ...
  function getSolution(exercise: SimpleSqlExerciseState): string {
  ...
  function getExerciseById(id: string): SimpleSqlExerciseState | null {
  ...
  function isExerciseValid(exercise: unknown): boolean {
    ...
    const exerciseId = (exercise as SimpleSqlExerciseState).id;
    ...
    const currentVersion = normalizeVersion((exercise as SimpleSqlExerciseState).version);
